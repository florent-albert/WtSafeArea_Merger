...
{
	//private static File CSS = new File("D:/PTC/VM_Shared_Folder/workspaces/WtSafeArea_Merger/codebase/com/ptc/ssp/wtsafeareamerger/codeReport.css");
	...
	public String getCodeDiffReport() throws IOException
	{
		...
		//htmlReport.append(FileUtils.readFileToString(CSS));
		...
		List<String> originalFileLines = fc.fileToLines(original);
        List<String> revisedFileLines = fc.fileToLines(revised);
	...
	public static String getDiffTOC(ArrayList<String> listFiles) throws IOException
	{
		...
		{
			String sourcePath = fileName.substring(fileName.indexOf("siteMod")+"siteMod".length()+1);
			sourcePath = sourcePath.replaceAll(Matcher.quoteReplacement(File.separator), "/");
	...
	private String printDiff(String source, int sourceLineNumber, String target, int targetLineNumber, Delta.TYPE deltaType)
	{
    	...
    	case CHANGE: //change = lines differs
    		String diffSourceTarget = org.apache.commons.lang3.StringUtils.difference(source, target);
    		String diffTargetSource = org.apache.commons.lang3.StringUtils.difference(target, source);
